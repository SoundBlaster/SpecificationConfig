...
1. Determine the set of candidate tasks:
   ...
   - Otherwise: choose from PRD §9 task IDs (A1…G3) that are not marked complete elsewhere.
2. Respect dependencies if they are tracked (Workplan dependency notes or PRD tables).
3. Prefer highest priority first (PRD tables: Priority column), then earlier phase order (A → G), then numeric order.
...
{1–3 sentences, copied from PRD §9 row (or Workplan)}
