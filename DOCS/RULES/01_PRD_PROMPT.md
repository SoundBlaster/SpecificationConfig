...
## Project-Specific Additions (SpecificationConfig)

When authoring task PRDs for this repository:

- Treat `DOCS/PRD/SpecificationConfig_PRD.md` as the canonical source for requirements, terminology, and the work plan.
- Prefer task IDs from PRD §9 (A1…G3) and reuse the same names in filenames: `DOCS/INPROGRESS/{ID}_{Title}.md`.
- Verification commands must match the repo’s actual tooling/CI:
  - `swift build -v`
  - `swift test -v`
  - If SwiftFormat is available: `swiftformat --lint .`
- Use repository-real paths (`Sources/SpecificationConfig`, `Tests/SpecificationConfigTests`) unless the task explicitly
  adds new targets/modules as part of the change.
